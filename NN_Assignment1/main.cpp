...
void getHiddenError(float error[], float hiddenLayer[], float prev_error[], float hiddenWeights[], int array_rows, int array_cols);
...
void getHiddenError(float error[], float hiddenLayer[], float prev_error[], float hiddenWeights[], int array_rows, int array_cols) {
    ...
    for (int i = 0; i < array_rows; i++) {
        ...
        for (int j = 0; j < array_cols; j++) {
            ...
            sumMultiplicationResult += prev_error[j] * hiddenWeights[(i * array_cols) + j];
        }
